...
def baseline_mean_errors(df, mean=True):
    ...
    if mean:
        ...
        # SSE
        SSE_bl = mean_squared_error(df.actual, df.baseline) * len(df)
        # MSE
        MSE_bl = mean_squared_error(df.actual, df.baseline)
        # RMSE
        RMSE_bl = sqrt(mean_squared_error(df.actual, df.baseline))
